...
The Bootstrap 3 compatibility build and original showcase compile with GWT 2.13.1. The Bootstrap 5 module is an initial native widget track with a small compiled showcase. The TeaVM modules are experimental and currently contain compile-checked subsets for Bootstrap 3-compatible and Bootstrap 5-native rendering.
...
`BOOTSTRAP5-PORTING.md` tracks Bootstrap 5 coverage against the current GwtBootstrap3 top-level widget catalogue. The Bootstrap 5 module is intentionally skeletal at this stage; the compatibility story belongs to Bootstrap 3, not to a Bootstrap 5 shim.

...
import org.gwtbootstrap5.client.ui.BootstrapJs;
...
public abstract class AbstractTooltip implements IsWidget, HasWidgets, HasOneWidget, HasId, HasHover {
    ...
    public void destroy() {
        if (widget != null) {
            BootstrapEventBridge.unbindAll(widget.getElement());
            BootstrapJs.dispose(getBootstrapPluginName(), widget.getElement());
        }
        initialized = false;
        showing = false;
    }
    ...
    protected final void initializePlugin(String content) {
        ...
        bindEvents(element);
        BootstrapJs.initializePlugin(element, getBootstrapPluginName(), animated, html,
    ...
    protected final void invokePlugin(String method) {
        if (widget != null) {
            BootstrapJs.call(getBootstrapPluginName(), widget.getElement(), method);
        }
    }
